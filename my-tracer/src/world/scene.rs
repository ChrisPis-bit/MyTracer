...
impl Scene{
    pub fn new(width: i32, height:i32, skybox_path : &str) -> Scene{
        ...
        Scene{
            ...
            lights: vec![
                Light::Point(PointLight::new(0, vec3(2000.0, 2000.0, 2000.0), point3(0.0, 50.0, 7.0)))
            ],
    ...
    pub fn update(&mut self){
        for x in 0..self.width as usize {
            for y in 0..self.height as usize {
                ...
                self.pixels[x + y * self.width as usize] = Scene::rgbf32_to_rgb8((color + Scene::rgb8_to_rgbf32(self.pixels[x + y * self.width as usize]) * (self.accumulated as f32)) / (self.accumulated as f32 + 1.0));
    ...
    pub fn ray_color(&mut self, ray: &mut Ray) -> Vector3<f32>{
        ...
        let light = self.sample_random_light();
        let light_col = light.get_albedo();
        let mut L = light.get_position() - I;
    ...
    pub fn sample_random_light(&mut self) -> &Light {
        let random_index = self.rand.gen_range(0..self.lights.len());
        &self.lights[random_index]
    }
